...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        out = self.block1(x)
        out = self.block2(out)
        return self.stocasthic_depth(out) + x
...
class Generator(nn.Module):
    def __init__(self, in_channels, features=64, num_resduals=9):
        super().__init__()
        ...
        self.residuals = nn.Sequential(*[ResidualBlock(features*16) for _ in range(num_resduals)])
        ...
        self.up4 = Block(features*2*2, features*1, stride=1, act="relu")
    ...
    def forward(self, x):
        ...
        up2 = self.up2(F.interpolate(torch.cat([up1, d4], dim = 2), scale_factor=2, mode="nearest"))
        up3 = self.up3(F.interpolate(torch.cat([up2, d3], dim = 2), scale_factor=2, mode="nearest"))
        up4 = self.up4(F.interpolate(torch.cat([up3, d2], dim = 2), scale_factor=2, mode="nearest"))
